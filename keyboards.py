...
def users_list_kb(users: list[dict], offset: int, has_more: bool) -> InlineKeyboardMarkup:
    ...
    for u in users:
        ...
        b.row(InlineKeyboardButton(text=f"{dot} {u['username']}{status_icon}", callback_data=f"user:{u['username']}"))
    nav = []
    if offset > 0:
        nav.append(InlineKeyboardButton(text="⬅️ قبلی", callback_data=f"list_users:{max(offset-10,0)}"))
    if has_more:
        nav.append(InlineKeyboardButton(text="بعدی ➡️", callback_data=f"list_users:{offset+10}"))
    if nav:
        b.row(*nav)
...
def user_detail_kb(username: str, status: str = "active") -> InlineKeyboardMarkup:
    ...
    status_toggle = "⛔️ غیرفعال‌سازی" if status == "active" else "✅ فعال‌سازی"
