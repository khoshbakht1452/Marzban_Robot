...
from config import BOT_TOKEN, ADMIN_IDS, MARZBAN_URL
from marzban_api import marzban, MarzbanError
from keyboards import main_menu_kb, users_list_kb, user_detail_kb, confirm_kb, cancel_kb
from states import AddUser, ExtendUser, AddDataUser
...
async def add_user_expire(message: Message, state: FSMContext):
    ...
    await state.clear()
    try:
        user = await marzban.create_user(username, gb, days)
    except MarzbanError as e:
        await message.answer(f"❌ خطا در ساخت کاربر:\n{e}", reply_markup=main_menu_kb())
        return
...
async def cb_user_detail(call: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await call.answer()
        return
    await call.message.edit_text(user_summary(user), reply_markup=user_detail_kb(username, user.get("status", "active")), parse_mode="HTML")
    await call.answer()
...
async def extend_days(message: Message, state: FSMContext):
    ...
    try:
        ...
        return
    await message.answer(f"✅ انجام شد.\n\n{user_summary(user)}", reply_markup=user_detail_kb(username, user.get("status", "active")), parse_mode="HTML")
